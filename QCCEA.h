#ifndef DBFGSP_NEW_QCCEA_H
#define DBFGSP_NEW_QCCEA_H
#pragma once
...
public:
    ...
    int RunEvolution(int CPUTime, vector<vector<Individual>> &QCCEAFinalAfterRepParetoSet, int AN, int mu);
...
private:
    ...
    int DetermineAction(int state, const vector<vector<double>> &Q) const;

    static vector<int> findMaxIndices(const vector<double> &vec);

    static int DetermineState( int ObjectMS, float ObjectTEC, const vector<Individual> &QCCEAPopulation);

    void UpdateQ(int oldstate, int newstate, int act, double reward, vector<vector<double>> &Q) const;
